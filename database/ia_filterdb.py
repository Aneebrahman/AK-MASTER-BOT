...
async def save_file(media):
    ...
    file_name = re.sub(r"(_|\+ |\-|\.|\+|\(@[A-Za-z]+_[A-Za-z]+\)|\(@[A-Za-z]+-[A-Za-z]+\)|\(@[A-Za-z]+_[A-Za-z0-9]+\)|\(@[A-Za-z0-9]+_[A-Za-z0-9]+\)|\(@[A-Z]+_[A-Z]+\)|\(@[A-Z]+-[A-Z]+\)|\[@[A-Z]+-[A-Z]+\]|\[@[A-Z]+\]|\[[A-Z]+\]|\([A-Za-z]+\)|\@[A-Za-z]+|\@[A-Z]+|\[MM\]\s|\[MM\]_|\[[A-Z]+\]|\@TvSeriesBay|\@Cinema\sCompany|\@Cinema_Company|\@CC_|\@CC|\@MM_New|\@MM_Linkz|\@MOVIEHUNT|\@CL|\@FBM|\@CKMSERIES|www_DVDWap_Com_|MLM|\@WMR|\[CF\]\s|\[CF\]|\@IndianMoviez|\@tamil_mm|\@infotainmentmedia|\@trolldcompany|\@Rarefilms|\@yamandanmovies|\[YM\]|\@Mallu_Movies|\@YTSLT|\@DailyMovieZhunt|\@I_M_D_B|\@CC_All|\@PM_Old|Dvdworld|\[KMH\]|\@FBM_HW|\@Film_Kottaka|\@CC_X265|\@CelluloidCineClub|\@cinemaheist|\@telugu_moviez|\@CR_Rockers|\@CCineClub|\[KC\]|\[AML\]|\[\@MOVIES HUNT\]|\[PM\]|\@WorldCinemaToday|\@MoviezzClub|\[CT™️\]|\@FILIMHOUSE|\@DMovies|MoviezzClub|\@MaasFile|\@CKMovies|\@KannadaWarriors|\@TG UPDATES1|\@UCDump|\@favio|\@MovieWorld2000|\@RatedRMovies|\@moviesdeveloper|\@RickyChannel|\@MoviesWar|\@AVA|\[ᎡᴛᏴᴛ\]|\[CC\]|\[MWK\]|\[PFM\])", " ", str(media.file_name))
    try:
        file = Media(
            file_id=file_id,
            file_ref=file_ref,
